...
class policy_value_network(object):
    ...
    def train_step(self, it, learning_rate=0):
        ...
        if True:
            ...
            # defensive step 3 check NaN See: https://stackoverflow.com/questions/40701712/how-to-check-nan-in
            # -gradients-in-tensorflow-when-updating grad_check = [tf.debugging.check_numerics(g, message='NaN
            # Found!') for g in clipped_grads] with tf.control_dependencies(grad_check):
            # self.optimizer.apply_gradients( zip(clipped_grads, self.model.trainable_variables),  # [v for _,
            # v in grads] global_step=self.global_step, name='train_step')
